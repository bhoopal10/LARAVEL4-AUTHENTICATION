...
// echo $sql;print_r($param);exit;
    ...
    {
        $values=Input::old();
        $judgeName=rtrim(Input::old('judgename'),',');
        $category=rtrim(Input::old('desc3'),',');
        $reference=rtrim(Input::old('references'),',');
        $reference1=rtrim(Input::old('references1'),',');
        $advocate=rtrim(Input::old('AdvName'),',');
        $ids=Input::old('ids');
        $sql="select * from `case` ";
        $param=array();
